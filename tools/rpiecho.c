	...
	while (1) {
		...
		/* Client-side Code */
			...
			if ((hal_time_ms() - tx_stamp) > timeout) {
				...
				tx.data.count = count;
				nrf24l01_set_ptx(spi_fd, tx_pipe);
				tx_status = nrf24l01_ptx_data(spi_fd, tx.buffer, MSG_SIZE);
				if (tx_status == 0) {
					tx_status = nrf24l01_ptx_wait_datasent(spi_fd);
					if (tx_status == 0) {
						print_buffer(1, PIPE, MESSAGE_SIZE, count, MESSAGE);
						attempt = 0;
						++count;
					} else
						++attempt;
			...
			if (pipe != NRF24_NO_PIPE) {
				...
				if (rx_len != 0) {
					print_buffer(0, pipe, rx_len, rx.data.count, rx.data.msg);
				}
			}
		}
	}
